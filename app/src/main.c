...
uint32_t policy_cb_get_rdo(const struct device *dev) {
  ...
  // TODO: Find out which source PDO lists the voltage we want. 1 is special for
  // 5v or something
...
const char *
usbc_policy_notify_to_str(const enum usbc_policy_notify_t policy_notify) {
...
void notify_cb(const struct device *dev,
  ...
  case PORT_PARTNER_NOT_RESPONSIVE:
    ...
    break;
  default:
    LOG_WRN("Unhandled notification: %s",
            usbc_policy_notify_to_str(policy_notify));
    break;
  }
}
